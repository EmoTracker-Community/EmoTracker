    ...
    public class IconUtility : ObservableSingleton<IconUtility>
    {
            ...
            for (int y = 0; y < bmp.PixelHeight; ++y)
            {
                for (int x = 0; x < bmp.PixelWidth; ++x)
                {
                    ...
                    switch (mode)
                    {
                        ...
                        case LuminanceMode.Average: b = g = r = (byte)((b + g + r) / 3.0); break;
                        case LuminanceMode.Max: b = g = r = Math.Max(Math.Max(b, g), r); break;
                        case LuminanceMode.Blue: g = r = b; break;
                        case LuminanceMode.Green: b = r = g; break;
                        case LuminanceMode.Red: b = g = r; break;
                        case LuminanceMode.BT709: b = g = r = (byte)(((uint)r + (uint)r + (uint)b + (uint)g + (uint)g + (uint)g) / 6); break;
                        case LuminanceMode.BT601: b = g = r = (byte)(((uint)r + (uint)r + (uint)b + (uint)g + (uint)g + (uint)g) >> 3); break;
                    }
            ...
            if (args.Length >= 1)
            {
                ...
                if (float.TryParse(args[0], System.Globalization.NumberStyles.Float, System.Globalization.CultureInfo.InvariantCulture, out brightness))
                {
                    ...
                    for (int y = 0; y < bmp.PixelHeight; ++y)
                    {
                        for (int x = 0; x < bmp.PixelWidth; ++x)
                        {
                            buffer[(y * bmp.BackBufferStride + x * 4) + 0] = (byte)(Math.Min(255.0f, Math.Max(0.0f, buffer[(y * bmp.BackBufferStride + x * 4) + 0] * brightness)));
                            buffer[(y * bmp.BackBufferStride + x * 4) + 1] = (byte)(Math.Min(255.0f, Math.Max(0.0f, buffer[(y * bmp.BackBufferStride + x * 4) + 1] * brightness)));
                            buffer[(y * bmp.BackBufferStride + x * 4) + 2] = (byte)(Math.Min(255.0f, Math.Max(0.0f, buffer[(y * bmp.BackBufferStride + x * 4) + 2] * brightness)));
                        }
                    }
        ...
        public static ImageSource MakeImageDim(ImageSource image, int divisor = 2)
        {
            ...
            for (int y = 0; y < bmp.PixelHeight; ++y)
            {
                for (int x = 0; x < bmp.PixelWidth; ++x)
                {
                    buffer[(y * bmp.BackBufferStride + x * 4) + 0] = (byte)(buffer[(y * bmp.BackBufferStride + x * 4) + 0] - buffer[(y * bmp.BackBufferStride + x * 4) + 0] / divisor);
                    buffer[(y * bmp.BackBufferStride + x * 4) + 1] = (byte)(buffer[(y * bmp.BackBufferStride + x * 4) + 1] - buffer[(y * bmp.BackBufferStride + x * 4) + 1] / divisor);
                    buffer[(y * bmp.BackBufferStride + x * 4) + 2] = (byte)(buffer[(y * bmp.BackBufferStride + x * 4) + 2] - buffer[(y * bmp.BackBufferStride + x * 4) + 2] / divisor);
                }
            }
