...
namespace SuperfastBlur
{
    public class GaussianBlur
    {
        ...
        Int32Rect mRect;
        WriteableBitmap mSrc;

        public GaussianBlur(WriteableBitmap image)
        {
            _width = image.PixelWidth;
            _height = image.PixelHeight;

            mRect = new Int32Rect(0, 0, _width, _height);
            mSrc = image;

            var source = new int[image.PixelWidth * image.PixelHeight];
            ...
            unsafe
            {
                fixed (int* pArray = source)
                {
                    ...
                    image.CopyPixels(mRect, intPtr, _width * _height * 4, image.BackBufferStride);
                }
            }            
        ...
        public void Process(int radial)
        {
            ...
            unsafe
            {
                fixed (int* pArray = dest)
                {
                    IntPtr intPtr = new IntPtr((void*)pArray);
                    mSrc.WritePixels(mRect, intPtr, _width * _height * 4, mSrc.BackBufferStride);
                }
            }
